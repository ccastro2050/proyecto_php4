<?php
/**
 * ControladorCliente — la capa HTTP de `cliente`.
 *
 * Lee la petición, VALIDA la forma del body (→ 422), delega al servicio y
 * responde JSON. Aquí no hay SQL ni reglas de negocio.
 *
 * LO QUE ESTA ENTIDAD AGREGA A LA v1: tiene **llaves foráneas** (`fkcodpersona` → `persona` y `fkcodempresa` → `empresa`).
...
require_once __DIR__ . '/../servicios/IServicioCliente.php';
...
class ControladorCliente
{
    ...
    // ------------------------------------------------------------------
    // GET /api/cliente[?limite=N]  →  listar
    // ------------------------------------------------------------------
    public function listar(): void
    {
        ...
        try {
            ...
            $this->responder(404, [
                'estado' => 404, 'mensaje' => 'Cliente no encontrado.', 'detalle' => $e->getMessage(),
            ]);
    ...
    public function crear(array $body): void
    {
        ...
        try {
            ...
            $this->responder(200, [
                'estado' => 200, 'mensaje' => 'Cliente creado exitosamente.',
                'id' => $idNuevo,
            ]);
        } catch (InvalidArgumentException $e) {
            $this->responder(400, [
    ...
    public function reemplazar(int $id, array $body): void
    {
        ...
        try {
            ...
            $this->responder(200, [
                'estado' => 200, 'mensaje' => 'Cliente reemplazado exitosamente.',
    ...
    // ------------------------------------------------------------------
    // PATCH /api/cliente/{id}  →  actualización PARCIAL
    // ------------------------------------------------------------------
    public function actualizar(int $id, array $body): void
    {
        ...
        try {
            $filas = $this->servicio->actualizar($id, $this->filtrarColumnas($body));
            $this->responder(200, [
                'estado' => 200, 'mensaje' => 'Cliente actualizado exitosamente.',
                'filasAfectadas' => $filas,
            ]);
        } catch (InvalidArgumentException $e) {
            $this->responder(400, [
    ...
    public function eliminar(int $id): void
    {
        try {
            ...
            $this->responder(200, [
                'estado' => 200, 'mensaje' => 'Cliente eliminado exitosamente.',
    ...
    private function validarCampos(array $datos, bool $obligatorios): array
    {
        ...
        if (array_key_exists('credito', $datos)) {
            if ((!is_int($datos['credito']) && !is_float($datos['credito'])) || $datos['credito'] < 0) {
                $errores[] = 'El campo credito debe ser un número mayor o igual a 0.';
            }
        } elseif ($obligatorios) {
            $errores[] = 'El campo credito es obligatorio.';
        }
        ...
        // fkcodempresa es OPCIONAL en la tabla: acepta un valor o NULL. Que llegue
        // vacío no es un error — significa «sin empresa».
        if (array_key_exists('fkcodempresa', $datos) && $datos['fkcodempresa'] !== null) {
            if (!is_string($datos['fkcodempresa']) || trim($datos['fkcodempresa']) === '' || strlen($datos['fkcodempresa']) > 10) {
                $errores[] = 'El campo fkcodempresa debe ser un texto de 1 a 10 caracteres.';
            }
        }
    ...
    {
        $datos = [];
        if (array_key_exists('credito', $body)) {
            $datos['credito'] = $body['credito'];
        }
        if (array_key_exists('fkcodpersona', $body)) {
            $datos['fkcodpersona'] = $body['fkcodpersona'];
        }
        if (array_key_exists('fkcodempresa', $body)) {
            $datos['fkcodempresa'] = $body['fkcodempresa'];
        }
        return $datos;
    }
