...
require_once __DIR__ . '/../servicios/IServicioPersona.php';
...
class ControladorPersona
{
    public function __construct(
        private readonly IServicioPersona $servicio,
    ) {
    }
    ...
    public function listar(): void
    {
        ...
        try {
            ...
            $this->responder(404, [
                'estado' => 404, 'mensaje' => 'Persona no encontrada.', 'detalle' => $e->getMessage(),
            ]);
    ...
    public function crear(array $body): void
    {
        ...
        try {
            ...
            $this->responder(200, [
                'estado' => 200, 'mensaje' => 'Persona creada exitosamente.',
            ]);
        } catch (InvalidArgumentException $e) {
            $this->responder(400, [
                'estado' => 400, 'mensaje' => 'Parámetros inválidos.', 'detalle' => $e->getMessage(),
    ...
    public function reemplazar(string $codigo, array $body): void
    {
        ...
        // (nombre, email, telefono) deben venir todos. Omitir uno es 422, no
        ...
        try {
            ...
            $this->responder(200, [
                'estado' => 200, 'mensaje' => 'Persona reemplazada exitosamente.',
    ...
    public function actualizar(string $codigo, array $body): void
    {
        ...
        try {
            $filas = $this->servicio->actualizar($codigo, $this->filtrarColumnas($body));
            $this->responder(200, [
                'estado' => 200, 'mensaje' => 'Persona actualizada exitosamente.',
                'filasAfectadas' => $filas,
            ]);
        } catch (InvalidArgumentException $e) {
            $this->responder(400, [
    ...
    public function eliminar(string $codigo): void
    {
        try {
            ...
            $this->responder(200, [
                'estado' => 200, 'mensaje' => 'Persona eliminada exitosamente.',
    ...
    private function validarCampos(array $datos, bool $obligatorios): array
    {
        ...
        if (array_key_exists('email', $datos)) {
            if (!is_string($datos['email']) || trim($datos['email']) === '' || strlen($datos['email']) > 100) {
                $errores[] = 'El campo email debe ser un texto de 1 a 100 caracteres.';
            }
        } elseif ($obligatorios) {
        ...
        if (array_key_exists('telefono', $datos)) {
            if (!is_string($datos['telefono']) || trim($datos['telefono']) === '' || strlen($datos['telefono']) > 20) {
                $errores[] = 'El campo telefono debe ser un texto de 1 a 20 caracteres.';
            }
        } elseif ($obligatorios) {
