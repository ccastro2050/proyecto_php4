# Modelo de datos — Versión 4: la misma base, en tres dialectos

> **Versión 4** · Las tablas, las llaves foráneas, los triggers y los
> procedimientos son **exactamente los mismos** que en la v2. Lo que cambia es
> que ahora existen **tres veces**: en MariaDB, en PostgreSQL y en SQL Server.
>
...
Ahora son tres, y cada uno en su subcarpeta:

| Archivo | Motor | Líneas | Cómo se ejecuta |
|---|---|---|---|
| `db/mariadb/init.sql` | MariaDB 11 | ~1.300 | La imagen lo hace sola |
| `db/postgres/init.sql` | PostgreSQL 16 | ~1.060 | La imagen lo hace sola |
| `db/sqlserver/bdfacturas.sql` | SQL Server 2022 | ~1.480 | **Un contenedor aparte** (`init.sh`) |

**Los tres vienen provistos y se copian tal cual.** Crean las mismas 12 tablas,
...
A los scripts de PostgreSQL y de SQL Server hubo que hacerles **una** de las
dos correcciones que la v2 le hizo al de MariaDB: la guarda que impide
modificar una factura ya anulada, que ninguno de los dos tenía. La otra —el
`estado` faltante en el listado— solo la necesitaba MariaDB.

> **Que los tres scripts tuvieran defectos distintos es, en sí mismo, la
> lección.** Fueron escritos por separado a partir del mismo diseño, y por
> separado divergieron: a uno le faltaba un campo, a los tres una guarda, y
> ninguno tenía exactamente los mismos huecos.
>
> Cuando la misma regla vive en tres sitios, tarde o temprano dice tres cosas
> distintas. Aquí lo dijo, y solo se supo porque hubo que probar los tres.

**Y la diferencia que aparece antes de escribir una línea de PHP:** las
imágenes de MariaDB y PostgreSQL ejecutan solas cualquier `.sql` que
encuentren en `/docker-entrypoint-initdb.d/`. La de SQL Server **no tiene ese
mecanismo**, así que `db/sqlserver/init.sh` es un script que espera a que el
servidor responda, crea la base y corre el guion — desde un contenedor
aparte. Los tres motores no se dejan levantar igual.
...
```powershell
docker compose up -d mariadb postgres sqlserver sqlserver-init
```
...
- El SQL de las consultas corrientes… **entre MariaDB y PostgreSQL.** Con
  SQL Server eso se acabó: ver §8.2.

### 8.2 Lo que cambia, y obligó a escribir código distinto

| | MariaDB | PostgreSQL | SQL Server |
|---|---|---|---|
| Nombre de la base | `bdfacturas_mariadb_local` | `bdfacturas_postgres_local` | `bdfacturas_sqlserver_local` |
| Usuario | `paradigmas` | `paradigmas` | **`sa`**, con clave de política |
| Limitar filas | `LIMIT` | `LIMIT` | **`OFFSET/FETCH`**, y exige `ORDER BY` |
| Llave autogenerada | `AUTO_INCREMENT` + `lastInsertId()` | `SERIAL` + `RETURNING` | `IDENTITY` + **`OUTPUT INSERTED`** |
| Salida de un procedimiento | `OUT` + `@resultado` | `INOUT`, fila del `CALL` | variable declarada en el SQL + `SELECT` |
| Cómo se niega un procedimiento | `SIGNAL` (`45000`) | `RAISE EXCEPTION` (`P0001`) | `THROW` (`42000` + nº ≥ 50000) |
| Llave duplicada | `1062` | `23505` | `2627` / `2601` |
| Llave foránea | `1452` insertar, `1451` borrar | **`23503` para los dos** | **`547` para los dos** |
| Un `INT` leído | texto | **entero** | texto |
| Decimales de la fecha | ninguno | seis | **siete** |

La lista completa, con lo que cada diferencia obliga a hacer, está en
[3_plan.md](3_plan.md) §4.2.

**Mire la fila de la llave foránea.** En la v3 parecía que PostgreSQL era el
motor raro por usar un solo código para los dos sentidos, mientras MariaDB los
separaba. Con la tercera columna se ve que **el raro era MariaDB**: SQL Server
también usa uno solo.

Con dos puntos de comparación uno no distingue una regla de una coincidencia.
Con tres empieza a poder — y ésa es, en una frase, la razón de que esta
versión exista.

> **Tres motores que hacen lo mismo no exponen la misma información de la
...
> escribir un solo código que sirva para los tres sin mentir en alguno.

### 8.3 Lo que NO se comparte: los datos

Las tres bases son **independientes**. Arrancan con los mismos datos de
ejemplo, y a partir de ahí cada una lleva su vida: un cliente creado con
`MOTOR=mariadb` no aparece con `MOTOR=sqlserver`.

No es un defecto y está declarado en el alcance. Esta versión demuestra que
**el programa** funciona contra las tres, no que las tres sean la misma.
