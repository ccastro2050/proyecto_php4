...
RUN apt-get update && apt-get install -y --no-install-recommends curl gnupg2 apt-transport-…
WORKDIR /app
COPY . .
EXPOSE 8090
...
| `EXPOSE 8090` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
| `CMD ["php", "-S", "0.0.0.0:8090", "index.ph…` | **El comando que se ejecuta al encender** el contenedor | Si ese proceso termina, el contenedor se apaga |
...
**Sin compose**, para levantar este proyecto —que tiene **7 servicios**— hay
que escribir esto, en este orden, cada vez:

```powershell
# 1. Crear la red, para que los contenedores se encuentren por su nombre
docker network create proyecto_php4_default

# 2. mariadb
docker run -d --name mariadb --network proyecto_php4_default --restart unless-stopped `
  ...
  -p 13329:3306 mariadb:11

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. postgres
docker run -d --name postgres --network proyecto_php4_default --restart unless-stopped `
  ...
  -p 15465:5432 postgres:16-alpine

# 5. ESPERAR a que responda de verdad… mirándolo a ojo

# 6. sqlserver
docker run -d --name sqlserver --network proyecto_php4_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Paradigmas123!" `
  -e "MSSQL_PID=Developer" `
  -v mssqldata:/var/opt/mssql `
  -p 11474:1433 mcr.microsoft.com/mssql/server:2022-latest

# 7. ESPERAR a que responda de verdad… mirándolo a ojo

# 8. sqlserver-init
docker run -d --name sqlserver-init --network proyecto_php4_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Paradigmas123!" `
...
docker run -d --name api-facturas --network proyecto_php4_default --restart unless-stopped `
  ...
  -e "DB_DSN_SQLSERVER=sqlsrv:Server=sqlserver,1433;Database=bdfacturas_sqlserver_local;TrustServerCertificate=yes" `
  -e "DB_USUARIO_SQLSERVER=sa" `
  -e "DB_CLAVE_SQLSERVER=Paradigmas123!" `
  -v "${PWD}/api_facturas:/app" `
  -p 8090:8090 api-facturas

# 10. Construir la imagen de front-php y encenderla
docker build -t front-php ./front_php
docker run -d --name front-php --network proyecto_php4_default --restart unless-stopped `
  -e "URL_API=http://api-facturas:8090" `
  -v "${PWD}/front_php:/app" `
  -p 8088:8088 front-php

# 11. phpmyadmin
docker run -d --name phpmyadmin --network proyecto_php4_default --restart unless-stopped `
  -e "PMA_HOST=mariadb" `
  -e "PMA_USER=paradigmas" `
  -e "PMA_PASSWORD=paradigmas123" `
  -p 8104:80 phpmyadmin:latest

```

**10 comandos**, con sus flags, en un orden que no se puede equivocar.
...
```yaml
URL_API: http://api-facturas:8090
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8090` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8090` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8090:8090`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8090`, el front
...
```powershell
docker network inspect proyecto_php4_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_php4` produce
la red `proyecto_php4_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_php4-front-php-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_php4_default has active endpoints (name:"proyecto_php4-front-php-1" …)
```
